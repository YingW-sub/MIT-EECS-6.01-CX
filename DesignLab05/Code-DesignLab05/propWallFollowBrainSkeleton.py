...
class WallFollower(sm.SM):
    ...
    def getNextValues(self, state, inp):
        ...
        if inp < 0.5:   #   距离过近
            ...
            if -0.1 < self.k * (inp - desiredRight) * self.T < 0.1:  #   死区设置，提高系统稳定性
                return state, io.Action(fvel=forwardVelocity, rvel=0)   #   直行
            else:
                return state, io.Action(fvel=forwardVelocity, rvel=self.k * (inp - desiredRight))   #   距离过远
